...
import { adminLoginApi } from "../service/adminLoginService";
...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      const res = await adminLoginApi({ identifier, password });
      ...
      if (user.role !== "admin") {
        toast.error("Only admin accounts can sign in here", { id: toastId });
        setErrors({ identifier: "This account does not have admin access." });
        return;
      }
      ...
      <div className="min-h-screen flex flex-col lg:flex-row">
        ...
        >
          ...
          <div className="relative z-10 p-7 sm:p-10 lg:p-14">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-semibold tracking-[0.3px]">Medigo Admin</span>
            </div>
            ...
            <h1 className="text-3xl sm:text-[38px] font-black leading-tight mt-6 mb-5 max-w-[520px]">
              Manage orders, doctors, and medicines.
              <span className="text-white/80"> Admin control center.</span>
            </h1>

            <p className="text-white/75 text-sm leading-relaxed max-w-[520px]">
              Sign in with your administrator credentials to access the Medigo admin panel.
            </p>
          </div>
          ...
          <div className="relative z-10 px-7 pb-7 sm:px-10 sm:pb-10 lg:px-14 lg:pb-14 grid grid-cols-2 gap-3">
            {(
              [
                { title: "Secure", sub: "Admin-only access" },
                { title: "Fast", sub: "Dashboard controls" },
                { title: "Overview", sub: "Lower support load" },
                { title: "Review", sub: "Order visibility" },
              ] as const
            ).map((s) => (
              <div key={s.sub} className="bg-white/15 border border-white/10 rounded-xl p-4">
        ...
        <div className="flex-1 flex items-center justify-center bg-light p-5 sm:p-8">
          <div className="w-full max-w-[440px]">
            <SectionHeading
              title="Admin sign in"
              description="Enter your admin credentials to continue to the dashboard."
              align="left"
              className="mb-6"
            />
            ...
            <p className="text-center text-sm text-slate-600 mt-5">
              Need a regular account? <Link to="/login" className="text-primary font-semibold hover:underline">Sign in here</Link>
            </p>
          </div>
        </div>
      </div>
